...
pub trait AsResponse: Sized {
    ...
    fn as_response_in_array(s: Self) -> Response { Self::as_response(s) }
}
...
impl AsResponse for &str {
    ...
    fn as_response_in_array(s: Self) -> Response { Response::BulkString(s.to_string().into_bytes()) }
}
...
pub fn _sort_response(r: &mut Response) {
    ...
    v.sort_by(|a, b| match (a, b) {
        (Response::Nil, Response::Nil) => Ordering::Equal,
...
macro_rules! command_test {
    ...
    (@expand_tests $db:ident $cmd:literal => [$($expected:expr),* $(,)?] ignore_order; $($rest:tt)*) => {
        ...
        let mut expected = Response::Array(vec![$($crate::test_utils::AsResponse::as_response_in_array($expected)),*]);
    ...
    (@expand_tests $db:ident $cmd:literal => [$($expected:expr),* $(,)?]; $($rest:tt)*) => {
        $crate::command_test!(@expand_test $db $cmd => Response::Array(vec![$($crate::test_utils::AsResponse::as_response_in_array($expected)),*]); $($rest)*);
    };
    (@expand_tests $db:ident $cmd:literal => $expected:expr; $($rest:tt)*) => {
